...
// Busca dados do Anúncio + Dados detalhados do Prestador para a prévia
$stmt = $pdo->prepare("
    SELECT a.*, 
           u.nome AS prestador, 
...
// Busca a média de avaliações GERAL do prestador (em todos os seus anúncios)
$stmt_seller_rating = $pdo->prepare("
    SELECT AVG(av.nota) as media_geral, COUNT(av.id) as total_geral
    FROM avaliacoes av
    JOIN anuncios an ON av.anuncio_id = an.id
...
// Processa Envio de Nova Avaliação
...
// Processa Resposta do Profissional
...
// Busca Média e Lista de Avaliações DESTE anúncio
...
$stmt_list_av = $pdo->prepare("
    SELECT av.*, u.nome AS cliente 
    FROM avaliacoes av 
    JOIN usuarios u ON av.cliente_id = u.id 
    WHERE av.anuncio_id = :id ORDER BY av.data_avaliacao DESC
");
$stmt_list_av->execute([':id' => $id]);
$avaliacoes = $stmt_list_av->fetchAll();

// Busca Lista de Perguntas e Respostas
$stmt_pr = $pdo->prepare("
    SELECT pr.*, u.nome AS cliente 
    FROM perguntas_respostas pr 
    JOIN usuarios u ON pr.cliente_id = u.id 
    WHERE pr.anuncio_id = :id ORDER BY pr.data_pergunta DESC
");
...
<main class="container" style="padding-top: 40px; padding-bottom: 50px; max-width: 900px;">
    ...
    <!-- CARÃO DE PRÉVIA DO ANUNCIANTE (SOBRE O PRESTADOR) -->
    <div style="background: #fff; border: 1px solid var(--border-color); padding: 25px; border-radius: 10px; margin-bottom: 30px;">
        ...
        <div style="display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 20px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div>
                    ...
                    <p style="color: #64748b; font-size: 0.85rem; margin-bottom: 5px;">
                        <i class="far fa-calendar-alt"></i> Membro desde <?= date('m/Y', strtotime($anuncio['prestador_desde'])) ?>
                    </p>
            ...
            <!-- Ações Rápidas de Contato e Perfil -->
                ...
                <?php foreach ($perguntas as $pq): ?>
                    <div style="border-bottom: 1px solid var(--border-color); padding-bottom: 15px;">
                        ...
                        <?php elseif (isset($_SESSION['usuario_id']) && $_SESSION['usuario_id'] == $anuncio['prestador_id']): ?>
                            <!-- Formulário para o Dono do Anúncio Responder -->
    ...
    <div style="background: #fff; border: 1px solid var(--border-color); padding: 30px; border-radius: 10px;">
        ...
        <?php if (isset($_SESSION['usuario_id']) && $_SESSION['usuario_id'] != $anuncio['prestador_id']): ?>
            <form action="anuncio.php?id=<?= $id ?>" method="POST" style="margin: 20px 0; background: #f8fafc; padding: 15px; border-radius: 8px;">
                ...
                <div class="form-group">
                    <textarea name="comentario" rows="2" placeholder="Deixe um comentário sobre o atendimento..." style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 6px;"></textarea>
                </div>
